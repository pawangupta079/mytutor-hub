      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="grid gap-3 text-sm md:grid-cols-2">
            {[
              "Tue 6pm - Rahul (Math)",
              "Thu 7pm - Aisha (Physics)",
              "Fri 5pm - Arjun (Chemistry)",
              "Sat 11am - Neha (Biology)",
            ].map((s) => (
              <div
                key={s}
          ...
          <CardContent>
            <div className="grid grid-cols-7 gap-2 text-center text-xs text-foreground/60">
              ...
              {Array.from({ length: 7 * 6 }).map((_, i) => (
                <div
                  key={i}
                  className={`h-10 rounded-md border ${i % 3 === 0 ? "bg-primary/10" : "bg-white"}`}
                ></div>
              ))}
            </div>
